...
public class AdjListGraph<V, E> implements Graph<V, E> {
	...
	private HashMap<V,vertex> vertexList;
	...
	public AdjListGraph(boolean directed) {
		...
		vertexList = new HashMap<V,vertex>();
	}
	...
	public Vertex<V> opposite(Vertex<V> v, Edge<E> e)
		...
		edge edge = cleanEdge(e);
		Vertex<V> ret = null;
		for(Vertex<V> vert : edge.getEndpoints()) {
			if(!vert.equals(v)) {
				ret = vert;
			}
		}
	...
	public void removeVertex(Vertex<V> v) throws IllegalArgumentException {
		...
		vertexList.remove(vert.getElement());
	...
	public Iterable<Vertex<V>> vertices() {
		...
		for(Entry<V, vertex> entry : vertexList.entrySet()) {
			list.addLast(entry.getValue());
		}
		return list;
	}
	
	public HashMap<V,vertex> vertMap(){
		return vertexList;
	}
